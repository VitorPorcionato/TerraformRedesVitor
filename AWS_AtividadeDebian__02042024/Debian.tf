...
data "template_file" "user_data_ec2" {
  template = file("./Scripts/site-debian.sh")
}
...
resource "aws_instance" "Debian" {
  ...
  vpc_security_group_ids      = [aws_security_group.Sec-Linux-Debian.id]
...
###### GRUPO DE SERGURANÇA ######
resource "aws_security_group" "Sec-Linux-Debian" {
  name        = "Sec-Linux-Debian"
